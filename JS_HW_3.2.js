...
let uniqSet = Array.from(new Set(database.map(item => JSON.stringify(item)))).map(item => JSON.parse(item));
console.log (uniqSet);
...
users.forEach (el => {
    if (usersUniq.indexOf(JSON.stringify(el)) === -1) {
        usersUniq.push(JSON.stringify(el));
    }
});
let parseUniq = usersUniq.map(item =>JSON.parse(item));
console.log(parseUniq);
